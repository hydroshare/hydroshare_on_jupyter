...
const mapDispatchToProps = (dispatch: ThunkDispatch<{}, {}, any>) => {
  return {
    uploadNewDir: (dirPath: string, choice: string) =>
      dispatch(uploadNewDir(dirPath, choice)),
  };
};
...
class SelectDirModal extends React.Component<
  ReduxType,
  IDirectorySelectState,
...
> {
  ...
  public directoryChoice = (event: any) => {
    event.target.value === "Yes"
      ? this.setState({ showDirectorySelector: true, choice: "Yes" })
      : this.setState({ showDirectorySelector: false, choice: "No" });
  };
  submit = () => {
    {
    ...
    return (
      ...
      >
        ...
        <div onChange={directoryChoice}>
          <input id="Yes" type="radio" name="dirselect" value="Yes" />
          Use custom directory
          <br />
          <input id="No" type="radio" name="dirselect" value="No" />
          Use default directory <i>(&lt;path-to-home-dir&gt;/hydroshare</i>
        </div>
        <br />
        {showDirectorySelector && (
          <div>
            <label>Enter your custom directory path</label>
            <br />
            <input
              id="myFile"
              type="text"
              onChange={(event) => {
                this.setState({ dirPath: event.target.value });
              }}
            />
          </div>
        )}
        {this.props.dirErrorResponse && (
          <p className="error">{this.props.dirErrorResponse}</p>
        )}
      </Modal>
    );
  }
}
export default connect(mapStateToProps, mapDispatchToProps)(SelectDirModal);
